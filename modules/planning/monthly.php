...
$sql = "SELECT s.*, u.fullname, t.template_name, si.site_name, a.area_name FROM inspection_schedule s LEFT JOIN users u ON u.id=s.inspector_id LEFT JOIN checklist_templates t ON t.id=s.template_id LEFT JOIN sites si ON si.id=s.site_id LEFT JOIN areas a ON a.id=s.area_id WHERE MONTH(s.inspection_date)=? AND YEAR(s.inspection_date)=?";
$params = [$month, $year];
...
$sql .= " ORDER BY s.inspection_date";
...
<div class="col-sm-6 text-end"><a href="create_schedule.php" class="btn btn-success"><i class="fas fa-plus"></i> New Planning</a></div>
</div>
</div>
</section>
...
<div class="col-md-4"><label>&nbsp;</label><button class="btn btn-primary"><i class="fas fa-search"></i> Search</button><a href="monthly.php" class="btn btn-secondary">Reset</a></div>
</div>
</form>
</div>
</div>
...
<tr><th>Date</th><th>Day</th><th>Inspector</th><th>Checklist</th><th>Site</th><th>Area</th><th>Status</th><th>Actions</th></tr>
</thead>
<tbody>
...
<td><?php $color=['Planned'=>'info','Assigned'=>'primary','In Progress'=>'warning','Completed'=>'success']; ?><span class="badge bg-<?= $color[$row['status']] ?? 'secondary' ?>"><?= htmlspecialchars($row['status']) ?></span></td>
<td><a href="view_schedule.php?id=<?= $row['id'] ?>" class="btn btn-info btn-sm"><i class="fas fa-eye"></i></a> <a href="edit_schedule.php?id=<?= $row['id'] ?>" class="btn btn-warning btn-sm"><i class="fas fa-edit"></i></a> <a href="delete_schedule.php?id=<?= $row['id'] ?>" class="btn btn-danger btn-sm" onclick="return confirm('Delete this planning?')"><i class="fas fa-trash"></i></a></td>
</tr>
<?php endwhile; ?>
<?php else: ?>
